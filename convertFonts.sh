...
do
	if [ ! -f $i ]
	then
		echo "One of the files you want to convert ($1) is not
a file.  Bailing." 1>&2
  	exit 40
	fi
done
...
designed to run under Windows using Cygwin. Installation instructions 
...
Are you *sure* you want to do this [y/N]?"
  ...
  then
    echo "Removing files."
    rm -r *.eot *.svg *.woff old *.css hinted-*.ttf 2> /dev/null
...
doautohint() {
	...
	then
	  # This is the 
    ttfautohint  --strong-stem-width="" --windows-compatibility --components  $1 $FONT_PREFIX$1
  else
